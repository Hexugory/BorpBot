	...
	async run(msg, args) {
        ...
        if(!duelstats[msg.author.id]){
            ...
            if(args.id > duelstats[msg.author.id].items.length-1 || args.id < 0){
                ...
                if((args.sl === 4 && !duelstats[msg.author.id].items[args.id].moveset) || (args.sl < 4 && duelstats[msg.author.id].items[args.id].moveset)){
                    ...
                    if(args.sl === 4){
                        ...
                        duelstats[msg.author.id].moveset = duelstats[msg.author.id].items[args.id];
                        ...
                        if(duelstats[msg.author.id].equipped[args.sl-1] != null){
                            duelstats[msg.author.id].items.push(duelstats[msg.author.id].equipped[args.sl-1]);
                        }
                        duelstats[msg.author.id].equipped[args.sl-1] = duelstats[msg.author.id].items[args.id];
                        duelstats[msg.author.id].items.splice(args.id, 1);
                        msg.client.provider.set(msg.guild, "duelstats", duelstats);
                        return msg.reply(`\`\`\`diff\n! Equipped item to slot ${args.sl} !\`\`\``)
                    }
